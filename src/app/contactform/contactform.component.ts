    ...
    switch (this.data.dbops) {
      case DBOperation.create:
        //this._contactService.addContact(Global.BASE_USER_ENDPOINT + 'addContact', contactData).subscribe(
          this._contactService.addContact(Global.BASE_USER_ENDPOINT, contactData).subscribe(
          data => {
            // Success
            if (data.message) {
      ...
      case DBOperation.update:        
        //this._contactService.updateContact(Global.BASE_USER_ENDPOINT + 'updateContact', contactData.id, contactData).subscribe(
          this._contactService.updateContact(Global.BASE_USER_ENDPOINT, contactData.id, contactData).subscribe(
          data => {
            // Success
            if (data.message) {
              this.dialogRef.close('success');
      ...
      case DBOperation.delete:
        //this._contactService.deleteContact(Global.BASE_USER_ENDPOINT + 'deleteContact', contactData.id).subscribe(
          this._contactService.deleteContact(Global.BASE_USER_ENDPOINT, contactData.id).subscribe(
          data => {
            // Success
            if (data.message) {
              this.dialogRef.close('success');
